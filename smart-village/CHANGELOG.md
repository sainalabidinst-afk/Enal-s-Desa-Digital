...
### Fixed
- Frontend tailwindcss-animate ESM compatibility (changed require to import)
- Seed.ts schema mismatches (removed isVerified, placeOfBirth, education, maritalStatus, occupation fields)
- LoginDto updated to use emailOrPhone instead of email-only validation
- Rate limiting added to auth endpoints (5 requests/minute)
- Secrets moved to .env files (.env.docker, apps/web-admin/.env.local)

### Changed
- Dockerfile.backend updated to use node:20-bullseye
- docker-compose.yml secrets via env_file references
- Updated Citizen DTO to match schema fields

### Security
- Rate limiting on /auth endpoints to prevent brute force
- JWT secrets externalized to environment files
